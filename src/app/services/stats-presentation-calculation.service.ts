...
export class StatsPresentationCalculationService {
  ...
  private setDataInCorrectRow(tempArr: any[], percentage: number, isFirst10?: boolean) {
    ...
    switch (percentage) {
      case 100:
        if (isFirst10) {
          this.statsService.presentation[100].push(...tempArr);
        } else {
          this.statsService.presentation[100].push(tempArr[tempArr.length - 1]);
        }
        break;
      case 90:
        if (isFirst10) {
          this.statsService.presentation[90].push(...tempArr);
        } else {
          this.statsService.presentation[90].push(tempArr[tempArr.length - 1]);
        }
        break;
      case 80:
        if (isFirst10) {
          this.statsService.presentation[80].push(...tempArr);
        } else {
          this.statsService.presentation[80].push(tempArr[tempArr.length - 1]);
        }
        break;
      case 70:
        if (isFirst10) {
